...
function effortAxis(p: ModelProfile): Pick<PickOption, "levels" | "level" | "disabled" | "levelNote"> {
	...
	const disabled = (reasoning.forbidden ?? []).filter((f) => f.thinking === "default").map((f) => levels.indexOf(f.effort as (typeof levels)[number])).filter((i) => i >= 0);
	return {
		levels,
		...(level >= 0 ? { level } : {}),
				...
				const picked = await new Promise<PickResult | null>((resolve) => {
					ctx.input.panelAsk(
						...
						(v) => {
							...
							// with no second axis is byte-for-byte the old one.
			...
			if (arg === "") {
				...
				try {
					...
					if (profile === undefined) {
						...
						// riding along ("claude high: unavailable") reads as
						// though the level were part of the problem. The panel
						// began producing that token in OR-7; the typed path
						// could always produce it too.
						bodyLog(
							unavailableReason(profName, profile),
						);
					} else {
		...
		ctx.chainRef.current = ctx.chainRef.current.then(async () => {
			...
			try {
				...
				const result = await ctx.session.summarize({
					...
					// thinking off where the registry can vouch for turning it
					// off. A fixed 4,000 was failing live sessions with "the
					// summary turn ended with max_tokens"; a complete checkpoint
					// of the reported size measured 18,837.
					manualBudget: true,
					...(focus !== "" ? { focus } : {}),
					onStart: (info) => {
						compactInfo = info;
