 ...
 *                       chat turn)
 *   tier D  — nested CLI with REAL provider env: ALL THIRTEEN tarballs under
...
// intra-kiso deps cannot hide behind a flattened root (F group).
{
	...
	const tarballs = ["@vincemakes/kiso-core", "@vincemakes/kiso-evals", "@vincemakes/kiso-runtime", "@vincemakes/kiso-tools-node", "@vincemakes/kiso-tui-cells", "@vincemakes/kiso-tui"].map((n) => pack(stage, n));
	for (const tarball of tarballs) {
...
// ── tier C: the CLI's full dependency closure ──────────────────────────
{
	...
	installTier(
		...
		[
			...
			"@vincemakes/kiso-tui-cells", "@vincemakes/kiso-tui",
			// R-D 0.1.45: the four official extensions likewise land from
