...
注释不计入——解释可以充分,实现必须精悍。这道门是快照纪律,不是自调节棘轮:它只移动过两次,每次都经裁定修正,而常备的逃生口是**抽取**(ADR-0043)。今天内核停在 **2,145 / 2,200** 行。产品面自 Amendment 8 起走另一套体制——每次 check 打印以供观察,但从不让 check 失败,它们的保护转移到了架构门禁上。
